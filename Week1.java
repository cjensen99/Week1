...
public class Week1 {
	...
	public static void main(String[] args) {
		...
		String[] customers = new String[] {
				...
				"Naima Bray","Harley Neville","Sabah Rosas","Kade Mccartney","Bruce Valenzuela","Elif Ireland",
				"Nansi Joseph","KeirBetts","Harvie Barnard","Kady Whitworth","Sadiyah Levine","Sakina Cresswell",
				"Chantel Wu","Harriette Booth","Isla-Mae Edge","Daniela Shepherd","Cosmo Lam","Myron Archer",
				"Caspian Bradford","Elyse Fletcher","Halima Devlin","Humzah Madden","Bevan Donnelly","Piotr Rice",
				"Malikah Mejia","Mujtaba Bassett"
		};
		
		double[][] payments = create2dArray();
		...
		while(stop == false) {
			...
			switch(input) {
				case 1:
					...
					printReportHeader();
					printCustomerPaymentHistory(customers, payments, index);
					printReportBottom();
					break;
				case 2:
				...
				case 3:
					...
					for(int i = 0; i < 50; i++) {
						...
						if(countZero > 2) printCustomerPaymentHistory(customers, payments, i);
						countZero = 0;
					}
					printReportBottom();
					break;
	...
	private static int returnAccountIndex(double[][] payments, int accountNumber) {
		for(int i = 0; i < 50; i++) {
	...
	private static void printCustomerPaymentHistory(String[] customers, double[][] payments, int index) {
		int count = 0;
		System.out.printf("%-18s", customers[index]);
		System.out.printf(" %d", (int) payments[index][0]);
		for(int i = 1; i < 13; i++) {
			if(i > 0 && payments[index][i] == 0) count++;
			System.out.print(" " + payments[index][i]);
		}
		System.out.print(" " + returnStanding(count) + "\n");
	}
	
	private static void printReportBottom() {
		System.out.println("---------------------------------------------------------------------------------------");
	}
	
	private static String returnStanding(int count) {
